	...
	class PaypalsController  extends AppController{
		...
		function paycart(){
			...
			if(!empty($listingid) && $itemUserId != $userid){
				...
				$prefix = $this->Shiping->tablePrefix;
				$userDetails = $this->Shiping->query("SELECT * FROM ".$prefix."shipings Where country_id =".$cntryExist_code);
            ...
            if($total_itms > 0){
                ...
                $area_data2 = array();
              	foreach($itm_datas as $itm) {
                    if($itm['Item']['delivery_type'] == 'regular'){
                        $area = $itm['User']['city'];
                        $del_area_id = $this->Deliverycharge->find('all',array('conditions'=>array('Deliverycharge.name'=> $area)));
